...
export default function ThisCantBeReached() {
  ...
  let CenterWidth = 0;
  let CenterHeight = 0;
  // 2. Destructure the personal website link from the config
  const { personalInfo } = config;
  const personalWebsite = personalInfo.socialLinks.personalWebsite ? personalInfo.socialLinks.personalWebsite.replace('https://', '') : 'your-domain.com';
      ...
      <div className="relative w-full  flex flex-col space-y-4">
        ...
        >
          ...
          <span className="text-gray-500 text-md">
            <span className="font-bold">{personalWebsite} </span> {/* Use dynamic URL */}
            unexpectedly{" "}
            {ShowText ? (
              <motion.span
